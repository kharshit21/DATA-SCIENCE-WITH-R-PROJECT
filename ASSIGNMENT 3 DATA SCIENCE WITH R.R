...
ggplot(iris, aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
  ...
  theme_minimal()
#conclusion-The code above creates a scatterplot where each point represents a combination of Sepal.Length and Petal.Length, and the points are colored based on the Species. By examining the plot, you can draw conclusions about the relationships between Sepal.Length, Petal.Length, and the different species of iris flowers.
...
titanic <- read_csv("C:/Users/Admin/Downloads/titanic.csv")
...
filtered_data$Survival <- factor(ifelse(filtered_data$Survived == 1, "Survived", "Died"), levels = c("Survived", "Died"))

# Create the plot with color aesthetics and separate male and female within categories
finalP <- ggplot(data = filtered_data, aes(x = Fare, y = factor(Survival), fill = Sex)) +
